...
- **Mechanic carry-over table.**
  ...
  | Relationship system (`characterRelationships`) | **Built** | The inert original scaffold was replaced with the full crew-chemistry system in §5.5: shared outcomes move pair values, chemistry modifies checks, extreme pairs refuse work together, partnerships affect the crew's cut, and unused pairs cool toward neutral |
  | Equipment drops from heists | **Built** | Each target's `possible_loot` is rolled during job settlement, with chances configured under `payout`; found equipment is added to inventory and reported in the results ledger |
