...
**Status:** M0–M1 landed, plus a delegated job loop. The d20 rules engine, the
data model, the campaign session with save/load, and a crew/board/results UI
are in; the planning screen and the procedural floorplan (M2–M3) are not.
...
| `src/sim/` | Job resolution and the week, drawing from the run's seeded RNG |
...
```powershell
.\scripts\capture_ui.ps1 -Scenes crew,board,results
```
