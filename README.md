...
**Status:** M0–M2 landed. The d20 rules engine, the data model, the campaign
session with save/load, and the planning screen are in: a job can be planned
door by door, committed, and resolved end to end. The procedural floorplan and
the dice presentation (M3) are next.
...
```powershell
.\scripts\capture_ui.ps1 -Scenes crew,board,planning,results
```
